...
/* ===== www.dedykuncoro.com ===== */
	...
	die(json_encode($response));
		...
		if (!empty($username) && $password == $confirm_password){
			$num_rows = mysql_num_rows(mysql_query("SELECT * FROM table_sopir WHERE username='".$username."'"));

			if ($num_rows == 0){
				$query = mysql_query("INSERT INTO table_sopir (id, username, password) VALUES(0,'".$username."','".$password."')");
